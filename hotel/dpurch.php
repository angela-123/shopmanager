...
<html>
    ...
    <body>
        ...
        $jon = mysql_connect('localhost','staff','angela')or die('cant connect');
         ...
         $date = $_POST['date'];
         $fez = "select date,item,qtyin,unitcost,qtyin*unitcost as extended from storetrans where qtyin > 0 And date = '$date'";
         $fizz = "select SUM(qtyin * unitcost) as Total from storetrans where date = '$date'";
         $res = mysql_query($fez);
