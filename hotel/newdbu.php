...
$wela = "select username,password,page from users where username = '$user'";
	...
	{   print '<div id = "jim">';
        ...
        <?php
         ...
         $fed = "select itemname as item,SUM(qtyout*mult) As qty,SUM(qtyout*mult*unitprice) as extended from lookup  GROUP BY itemname";
         
         
         
         $sfed = "select itemname as item,qtyout ,qtyout*unitprice*mult  as extended from lookup    ";
         
         $jfed = "select itemname as item,qtyout ,SUM(qtyout*unitprice *mult) as extended from lookup  where qtyout > 0 GROUP BY itemname";
         ...
         if($res)
         {
             ...
             echo 'Sales Records Not Retrieved';
                ...
                echo '<tr>';
                echo '<td id = jili>';
                echo '<nobr>'. 'Total Sales For Period' .'</nobr>';
                echo '</td>' ;
                echo '</tr>';
                echo '<tr>';
                echo '<td>';
