...
‪
    ...
    <body>
        ...
        $dex = "select productname,sum(qtyout) as qty,unitprice,sum(qtyout * unitprice) as extended from sales where monthname(sdate) = '$date' and year(sdate) = '$yr' group by productname";
        $tara = "select sdate, sum(qtyout) as qtysold,sum(qtyout*unitprice) as sales,sum(paid) as payment,sum(deposit) as deposits,sum(others) as others from sales where monthname(sdate) = '$date' and year(sdate) = '$yr' group by year(sdate)";
